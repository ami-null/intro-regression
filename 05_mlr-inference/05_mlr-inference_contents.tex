...
\begin{frame}{Estimating $\sigma^2$}
    ...
    \begin{itemize}
        ...
        \item Normal divided by an independent $\sqrt{\chi^2/\text{df}}$ gives a $t$ distribution: this is the source of every $t$ and $F$ result today
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Goal for today: explain every number in this output}
    \begin{align*}
...
\begin{frame}{Question}
    We have fitted a model with regressors $x_1,\dots,x_k$. How do we decide whether \emph{any} of them is related to $y$?
    % \begin{itemize}
...
\begin{frame}{Question}
    In the simple regression of $y$ on $x_1$ alone, the slope estimate is $2.176$. In the multiple regression on $x_1$ and $x_2$, it is $1.616$. Why do they differ?
    % \begin{itemize}
...
\begin{frame}{Looking ahead}
    \begin{itemize}
        \item All of these procedures rest on the model assumptions (linearity, constant variance, independence, normality), which we check with residual analysis
        \item $t_0^2$ being a partial $F$ leads to extra sums of squares and $F$ tests for general linear hypotheses
        \item Correlated regressors inflate $\operatorname{se}(\hat\beta_j)$: collinearity
    \end{itemize}
\end{frame}
