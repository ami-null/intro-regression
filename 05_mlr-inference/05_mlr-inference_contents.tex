...
\begin{frame}{Question}
    ...
    \begin{itemize}
        ...
        \item The PI does not shrink to a point: it tends to $\hat y_0 \pm z_{\alpha/2}\sigma$, because a new observation still carries its own noise $\varepsilon_0$
    \end{itemize}
\end{frame}
...
\begin{frame}{CI and PI bands}
    \begin{columns}[T]
        ...
        \begin{center}
            \mode<presentation>{\def\tikzw{\linewidth}\def\tikzh{0.68\linewidth}}
            \mode<article>{\def\tikzw{0.85\linewidth}\def\tikzh{0.55\linewidth}}
            \begin{tikzpicture}
                \begin{axis}[
                    width=\tikzw, height=\tikzh,
...
\begin{frame}{Regressor variable hull}
    \begin{columns}[T]
        ...
        \column{0.6\textwidth}
        \begin{center}
            \mode<presentation>{\def\tikzw{\linewidth}\def\tikzh{0.72\linewidth}}
            \mode<article>{\def\tikzw{0.85\linewidth}\def\tikzh{0.5\linewidth}}
            \begin{tikzpicture}
                \begin{axis}[
                    width=\tikzw, height=\tikzh,
