...
\begin{frame}{From a line to a plane}
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            \begin{center}
                \mode<presentation>{\def\tikzrbw{\linewidth}}
                \mode<article>{\def\tikzrbw{0.5\linewidth}}
                \resizebox{\tikzrbw}{!}{%
...
\begin{frame}{Geometry of least squares}
    \(\mathbf{H}\) is symmetric and idempotent and \(\hat{\mathbf{y}} = \mathbf{H}\mathbf{y}\), therefore, \(\hat{\mathbf{y}}\) is the orthogonal projection of \(\mathbf{y}\) onto \(\mathbf{X}\).
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{itemize}
        ...
        \begin{column}{0.48\textwidth}
            \begin{center}
                \mode<presentation>{\def\tikzscale{0.9}\def\tikzrbw{\linewidth}}
                \mode<article>{\def\tikzscale{0.95}\def\tikzrbw{0.5\linewidth}}
                \resizebox{\tikzrbw}{!}{%
...
\begin{frame}{SLR variance from the MLR covariance matrix}
